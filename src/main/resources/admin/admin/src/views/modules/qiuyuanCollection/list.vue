        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '球员姓名' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.qiuyuanName" placeholder="球员姓名" clearable></el-input>
                    </el-form-item>
                                         
                                                                                                                                             
                    <el-form-item :label="contents.inputTitle == 1 ? '球员位置' : ''">
                        <el-select v-model="searchForm.qiuyuanTypes" placeholder="请选择球员位置">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                                    v-for="(item,index) in qiuyuanTypesSelectSearch"
                                    v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('qiuyuanCollection','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('qiuyuanCollection','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('qiuyuanCollection','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('qiuyuanCollection','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/lanqiulianmengguanlxitong/upload/qiuyuanCollectionMuBan.xls"
                        >批量导入球员收藏数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('qiuyuanCollection','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="qiuyuanCollectionUploadSuccess"
                                :on-error="qiuyuanCollectionUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('qiuyuanCollection','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入球员收藏数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('qiuyuanCollection','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "qiuyuanCollection.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('qiuyuanCollection','查看')"
                          :data="dataList"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            {{scope.row.qiuyuanName}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="qiuyuanPhoto"
                               header-align="center"
                               width="200"
                               label="头像">
                        <template slot-scope="scope">
                            <div v-if="scope.row.qiuyuanPhoto">
                                <img :src="$base.url+scope.row.qiuyuanPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="qiuyuanCollectionTypes"
                                      header-align="center"
                                      label="类型">
                        <template slot-scope="scope">
                            {{scope.row.qiuyuanCollectionValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('qiuyuanCollection','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('qiuyuanCollection','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('qiuyuanCollection','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //级联表下拉框搜索条件
              ...
              qiuyuanTypesSelectSearch : [],
    //当前表下拉框搜索条件
            form:{
                id : null,
                qiuyuanId : null,
                yonghuId : null,
                qiuyuanCollectionTypes : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '体重': 'qiuyuanTizhong',
                     '年龄': 'qiuyuanAge',
                     '进球数': 'qiuyuanJinqiushu',
                     '助攻数': 'qiuyuanZhugong',
                     '获得奖杯': 'qiuyuanJiangbei',
                     '球员位置': 'qiuyuanTypes',
                     '用户姓名': 'yonghuName',
                     '用户手机号': 'yonghuPhone',
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'qiuyuan_collection',//当前表表名,
                        sumColum : 'qiuyuan_collection_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'qiuyuan_collection_name',//分组字符串字段
                        // types : 'qiuyuan_collection_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "qiuyuan_collection_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                let params = {
                    ...
                    limit: this.pageSize,
                    sort: 'id',
                    qiuyuanCollectionTypes: 1,
                }

                                         
                if (this.searchForm.qiuyuanName!= '' && this.searchForm.qiuyuanName!= undefined) {
                    params['qiuyuanName'] = '%' + this.searchForm.qiuyuanName + '%'
                }
                ...
                if (this.searchForm.qiuyuanTypes!= '' && this.searchForm.qiuyuanTypes!= undefined) {
                    params['qiuyuanTypes'] = this.searchForm.qiuyuanTypes
                }
                ...
                params['qiuyuanCollectionDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "qiuyuanCollection/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                this.$http({
                    url: "dictionary/page?dicCode=qiuyuan_types&page=1&limit=100",
                    method: "get",
                    page: 1,
                    limit: 100,
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.qiuyuanTypesSelectSearch = data.data.list;
                    }
                });
                //查询当前表搜索条件所有列表
            },
            ...
            // 导入功能上传文件成功后调用导入方法
            qiuyuanCollectionUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "qiuyuanCollection/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            qiuyuanCollectionUploadError(data){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "qiuyuanCollection/update",
                        method: "post",
                        data: {
                            id:id,
//                            qiuyuanCollectionTypes:qiuyuanCollectionTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
